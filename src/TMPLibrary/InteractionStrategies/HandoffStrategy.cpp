...
operator()(Interaction* _interaction, State& _start) {
  ...
  for(size_t i = 2; i < finalStage; i++) {
    ...
    std::cout << "Start plan motions" << std::endl;
    ...
    std::cout << "Finish plan motions" << std::endl;
...
GenerateInitialState(Interaction* _interaction, const State& _previous, const size_t _next) {
  ...
  //std::cout << "ARTIFICIAL CENTER, " << x << ", " << y << ", " << z << std::endl;
    ...
    for(auto group : deliverers) {
      ...
      if(!m_physicalDemo) {
        ...
        for(auto r : group->GetRobots()) {
          ...
          if(c[0] < x+m_minX)
            c[0] = x+m_minX;
          else if(c[0] > x+m_maxX)
            c[0] = x+m_maxX;

          if(c[1] < y+m_minY)
            c[1] = y+m_minY;
          else if(c[1] > y+m_maxY);
            c[1] = y+m_maxY;

          if(c[2] < z+m_minZ)
            c[2] = z+m_minZ;
          else if(c[2] > z+m_maxZ)
            c[2] = z+m_maxZ;
          //std::cout << "HANDOFF CENTER, " << c[0] << ", " << c[1] << ", " << c[2] << std::endl;
...
GenerateTransitionState(Interaction* _interaction, const State& _previous, const size_t _next, MPSolution* _solution) {
  ...
  for(size_t i = 0; i < 1; i++) {
    ...
    for(auto kv : eeFrames) {
      ...
      if(!cfg.GetRobot()) {
        if(m_debug) {
          std::cout << "0 Failed to find a valid grasp pose for " << kv.first->GetLabel() << std::endl;
        }
      	failed = true;
        break;
      }
