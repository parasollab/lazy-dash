...
class CompositeGraph : public GenericStateGraph<Vertex, Edge> {
  ...
  public:
    ...
    virtual bool IsChecked(const VID _source, const VID _target, const int _res) noexcept override;

    /// Remove an edge from the graph if it exists.
    /// @param _iterator An iterator to the edge.
    virtual bool IsChecked(EI _iterator, const int _res) noexcept override;


    virtual void SetChecked(const VID _source, const VID _target, const int _res) noexcept override;

    /// Remove an edge from the graph if it exists.
    /// @param _iterator An iterator to the edge.
    virtual void SetChecked(EI _iterator, const int _res) noexcept override;
...
bool
CompositeGraph<Vertex, Edge>::
IsChecked(const VID _source, const VID _target, const int _res) noexcept {
  // Delete the individual edges.
  ED edgeDescriptor(_source, _target);
  VI dummy;
  EI edgeIterator;
  ...
  return IsChecked(edgeIterator, _res);
}
...
bool
CompositeGraph<Vertex, Edge>::
IsChecked(EI _iterator, const int _res) noexcept {
  ...
  for(size_t i = 0; i < m_group->Size(); ++i) {
    if(GetIndividualGraph(i)->IsChecked(descriptors[i].source(), descriptors[i].target(), _res))
      continue;
    isAllChecked = false;
    break;
  }
...
void
CompositeGraph<Vertex, Edge>::
SetChecked(const VID _source, const VID _target, const int _res) noexcept {
  // Delete the individual edges.
  ...
  return SetChecked(edgeIterator, _res);
}
...
void
CompositeGraph<Vertex, Edge>::
SetChecked(EI _iterator, const int _res) noexcept {
  // Execute pre-delete hooks and update vizmo debug.
  this->ExecuteDeleteEdgeHooks(_iterator);
  ...
  auto& descriptors = edge.GetEdgeDescriptors();
  for(size_t i = 0; i < m_group->Size(); ++i) {
    GetIndividualGraph(i)->SetChecked(descriptors[i].source(), descriptors[i].target(), _res);
  }
}
...
AddEdge(const VID _source, const VID _target, const Edge& _lp) noexcept {
  ...
  std::cout << "\nAdd Edge of " << m_group->GetLabel() << std::endl;
  std::cout << _source << " --->> " << _target << std::endl;
  ...
  for(size_t i = 0; i < edgeDescriptors.size(); ++i) {
    ...
    std::cout << "\t(" << i << ") " << individualSourceVID << ": " << sourceCfg << std::endl;
    std::cout << "\t(" << i << ") " << individualTargetVID << ": " << targetCfg << std::endl;
...
AddVertex(const Vertex& _v) noexcept {
  ...
  // std::cout << "\nAdd Vertex of " << m_group->GetLabel() << std::endl; 
  ...
  for(size_t i = 0; i < m_group->Size(); ++i) {
    ...
    // std::cout << "\t" << robot->GetLabel() << " (" << individualVID << "): " 
    //           <<  cfg.GetRobotCfg(i) << std::endl;
...
DeleteEdge(const VID _source, const VID _target) noexcept {
  ...
  // std::cout << "\n\t\t\tDelete in CompositeGraph " << m_group << std::endl;
  // std::cout << "\t\t\t\t(" << _source << "): " << this->GetVertex(_source) << std::endl;
  // std::cout << "\t\t\t\t(" << _target << "): " << this->GetVertex(_target) << std::endl;
...
DeleteEdge(EI _iterator) noexcept {
  ...
  auto& descriptors = edge.GetEdgeDescriptors();
  for(size_t i = 0; i < m_group->Size(); ++i) {
    if(descriptors[i].source() == descriptors[i].target())
      continue;
