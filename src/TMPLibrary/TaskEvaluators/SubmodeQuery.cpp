...
std::unordered_map<SemanticTask*,size_t>
SubmodeQuery::
GetVertexTasks() {
  return m_vertexTasks;
}

std::unordered_map<SemanticTask*,size_t>
SubmodeQuery::
GetHyperarcTasks() {
  return m_hyperarcTasks;
}
...
AddSchedulingConstraint(SemanticTask* _task, SemanticTask* _constraint) {
  ...
  // std::cout << "FIND " << _task->GetLabel() << std::endl;
  // std::cout << std::endl;
  
  // // Check if the _task is a vertex or a hyperarc
  // std::cout << "Vertex tasks size: " << m_vertexTasks.size() << std::endl;
  // for(auto kv : m_vertexTasks) {
  //   std::cout << kv.first->GetLabel() << ": " << kv.second << std::endl;
  // }
  // std::cout << std::endl;
  // std::cout << "Hyperarc tasks size: " << m_hyperarcTasks.size() << std::endl;
  ...
  for(auto elem : path) {
    ...
    for(auto stage : taskSet) {
      ...
      for(auto groupTask : stage) {
        ...
        // If this is active
        if(groupTask->GetRobotGroup()->Size() > 1 or !groupTask->GetRobotGroup()->GetRobots()[0]->GetMultiBody()->IsPassive()) {
          m_hyperarcTasks[task.get()] = aeh.property;
        }
        else {
          for(auto vid : aeh.head) {
            auto groundedVID = m_actionExtendedHypergraph.GetVertexType(vid).groundedVID;
            auto vertex = gh->GetVertex(groundedVID);
            if(vertex.first->GetGroup()->Size() > 1)
              continue;
            if(!vertex.first->GetGroup()->GetRobots()[0]->GetMultiBody()->IsPassive())
              continue;
...
ComputeHeuristicValues() {
  ...
  for(auto kv : m_costToGoMap) {
    ...
    if(m_debug) {
      ...
      auto vertex = gh->GetVertex(kv.first);
      if(vertex.first) {
        std::cout << vertex.first->GetGroup()->GetLabel() << " " << vertex.first->GetVertex(vertex.second).PrettyPrint() << std::endl;
      }
    }
  }
}
...
HyperpathQuery() {
  ...
  if(m_pq.empty()) {
    m_goalVID = MAX_UINT;
    auto output = SBTDijkstraWithConstraints(&m_actionExtendedHypergraph,0,weight,termination,forwardStar,heuristic);
    m_mbt = output.first;
    m_pq = output.second;
  }
  else {
...
HyperpathForwardStar(const size_t& _vid, ActionExtendedHypergraph* _h) {
  if(m_debug)
    std::cout << "hyperpath forwardstar: " << _vid << std::endl;
  if(m_computedFS.count(_vid)) {
    if(m_debug)
      std::cout << _vid << " is already computed" << std::endl;
  ...
  auto mth = mg->GetModeTransitionHypergraph();
  auto gh = dynamic_cast<GroundedHypergraph*>(this->GetStateGraph(m_ghLabel).get());
  //auto& gh = mg->GetGroundedHypergraph();
  ...
  RobotGroup* blockedGroup = nullptr;

  // Extra for blocking reverse actions
  std::vector<size_t> blockedVertices;

  auto vertexMode = mg->GetTransitionModeOfGroundedVID(groundedVID);
  auto vertexGroup = mth.GetVertex(vertexMode).property->robotGroup;
  ...
  if(incoming.size() > 0) {
    ...
    if(tail.size() == 1) {
      ...
      auto parentMode = mg->GetTransitionModeOfGroundedVID(parentVertex.groundedVID);
      auto parentGroup = mth.GetVertex(parentMode).property->robotGroup;
      // if(vertexMode == parentMode)
      //   blockedMode = vertexMode;
      if(parentGroup == vertexGroup)
        blockedGroup = vertexGroup;
    }
  ...
  for(auto hid : aev.history) {
    ...
    for(auto v : hyperarc.tail) {
      ...
      auto mvid = mg->GetTransitionModeOfGroundedVID(gvid);
      if(mvid == source) {
        continue;
      }
      if(mvid == MAX_UINT) {
      ...
      auto mode = mth.GetVertexType(mvid);
  ...
  for(auto hid : gh->GetOutgoingHyperarcs(groundedVID)) {
    ...
    bool monotonic = true;
    for(auto v : head) {
      auto mvid = mg->GetTransitionModeOfGroundedVID(v);
    ...
    if(head.size() == 1 and blockedGroup != nullptr) {
      auto groundedHead = *(head.begin());
      auto headModeVID = mg->GetTransitionModeOfGroundedVID(groundedHead);
      auto headGroup = mth.GetVertex(headModeVID).property->robotGroup;
      // if(headModeVID == blockedMode) {
      if(headGroup == blockedGroup) {
        if(m_debug)
          std::cout << "group are the same" << std::endl;
        continue;
      }
    }
...
HyperpathHeuristic(const size_t& _target) {
  ...
  auto parentMode = mg->GetTransitionModeOfGroundedVID(groundedParentVID);
  auto targetMode = mg->GetTransitionModeOfGroundedVID(vid);
  ...
  for(auto hid : outgoings) {
    ...
    for(auto v : ha.head) {
      auto m = mg->GetTransitionModeOfGroundedVID(v);
      if(m != targetMode) {
        outgoing = hid;
        break;
      }
...
CheckSchedulingConstraints(size_t _hid, std::set<size_t> _tail, ActionHistory _history, std::set<size_t>& _fullyGroundedHyperarcs) {
  if(m_debug)
    std::cout << "CheckSchedulingConstraints" << std::endl;
  ...
  for(auto vid : frontier) {
    ...
    for(auto h : incoming) {
      ...
      auto ha = m_actionExtendedHypergraph.GetHyperarc(h);
      bool recent = true;
      for(auto v : ha.head) {
        if(!frontier.count(v)) {
          recent = false;
      ...
      if(recent) {
        ...
        mostRecentHyperarcs.insert(ha.property);
      }
    }
  }
  ...
  std::cout << "$$$ " ;
  for(auto t : _tail) {
    std::cout << m_actionExtendedHypergraph.GetVertexType(t).groundedVID << ", ";
  }
  std::cout << " --->> " ;
  for(auto h : head) {
    std::cout << m_actionExtendedHypergraph.GetVertexType(h).groundedVID << ", ";
  }
  std::cout << std::endl;
