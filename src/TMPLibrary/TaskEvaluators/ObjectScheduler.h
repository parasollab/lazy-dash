#ifndef PPL_OBJECT_SCHEDULER_H_
#define PPL_OBJECT_SCHEDULER_H_
...
class ObjectScheduler : public TaskEvaluatorMethod {
  ...
  public:
    ...
    // typedef GroupLocalPlan<Cffg>                              GroupWeightType;
    ...
    std::unordered_map<size_t,std::set<size_t>> GetTaskSpaceImprovementCandidates();
    std::set<std::pair<size_t,size_t>> GetExtraTaskSpaceCandidates();
  ...
  private:
    ...
    std::set<std::pair<size_t,size_t>> m_extraTaskSpaceCandidateSet;

    std::set<std::pair<size_t,size_t>> m_prevGeometricConstraintSet;
