...
LidGraspStrategy::
LidGraspStrategy() {
  this->SetName("LidGraspStrategy");
}

LidGraspStrategy::
LidGraspStrategy(XMLNode& _node) : GraspStrategy(_node) {
  this->SetName("LidGraspStrategy");
...
operator()(Interaction* _interaction, State& _start) {
  std::cout << "sssss " << _start.size() << std::endl;
  ...
  for(size_t i = 1; i < graspStage; i++) {
    ...
    // Add composite groups to grasp solution
    ...
    auto eeFrames = ComputeEEFrames(_interaction,objectPoses,i,"lid");
    ...
    */
    ...
    for(auto kv : eeFrames) {
      std::cout << "robot... " << kv.first->GetLabel() << std::endl;
      ...
      SetEEDOF(_interaction,kv.first,cfg,stages[i]);
    ...
    auto nextStageEEFrames = ComputeEEFrames(_interaction,objectPoses,i+1,"lid");
    ...
    for(auto kv : nextStageEEFrames) {
      ...
      SetEEDOF(_interaction,kv.first,cfg,stages[i+1]);
    ...
    std::cout << "__ pregrasp constraints __ " << std::endl;
    ...
    std::cout << "__ grasp constraints __ " << std::endl;
    ...
    auto toGraspPaths = PlanMotions(graspTasks,toStageSolution,
        "PlanInteraction::"+_interaction->GetLabel()+"::"+stages[i+1],
    ...
    std::cout << ">>>>>>>> " << stages[i] << " --> " << stages[i+1] << " finished " << std::endl;
  }
  ...
  for(auto kv : _start) {
    ...
    if(active and passive){ 
      Cfg objectPose(passive);
      objectPose = gcfg.GetRobotCfg(passive);
      ...
      objectPose[4] = 0;
      objectPose[5] = 0;
      objectPoses[passive] = objectPose;
      objectPose.ConfigureRobot();
      std::cout << "active and passive: " << objectPose.PrettyPrint() << std::endl;
      // constraintMap[passive] = std::unique_ptr<CSpaceConstraint>(new CSpaceConstraint(passive,objectPose));

    }
    else{ // object
      auto objectPose = gcfg.GetRobotCfg(passive);
      objectPoses[passive] = objectPose;
      objectPose.ConfigureRobot();
      std::cout << "bottom object pose: " << objectPose.PrettyPrint() << std::endl;
    }
  }


  // Compute goal pose for robot based of transform and initial object pose
  auto nextStageEEFrames = ComputeEEFrames(_interaction,objectPoses,graspStage+1,"lid");
  for(auto kv : nextStageEEFrames) {
    auto robot = kv.first; // Ref robot
    ...
    SetEEDOF(_interaction,kv.first,cfg,stages[graspStage]);
  ...
  for(auto kv : _start) {
    ...
    if(active) {
      ...
      Cfg objectCfg = objectPoses[passive];
      gcfg.SetRobotCfg(passive,std::move(objectCfg));
      // Cfg robotCfg = robotCfgs[active];
      // gcfg.SetRobotCfg(active,std::move(robotCfg));
      std::cout << "goal constraints " << gcfg.PrettyPrint() << std::endl;
  ...
  std::cout << "____ goal constraints ____" << std::endl;
  ...
  */
  ...
  // Plan path
  std::cout << "____ Planning Motions ____" << std::endl;
  ...
  std::cout << "____ Planning Done ____" << std::endl;
  ...
  if(delay > 0) {
    ...
    for(auto path : paths) {
      ...
      // if(wait.empty()) {
      //   std::cout << "wait empty" << delay << std::endl;
      //   auto last = vids.back();
      ...
      // }
      // else {
      //   std::cout << "wait not empty" << delay << std::endl;
  ...
  std::cout << "~~" << std::endl;
  _start = InterimState(_interaction,stages[graspStage+1],stages[graspStage+1],paths);
...
AssignRoles(const State& _state, const std::vector<std::string>& _conditions) {
  ...
  for(auto condition : _conditions)
      std::cout << "condition: " << this->GetTMPLibrary()->GetActionSpace()->GetCondition(condition)->GetLabel() << std::endl;
  ...
  if(!validStart) {
    ...
    std::cout << "valid condition" << std::endl;
...
Cfg
LidGraspStrategy::
SampleObjectPose(Robot* _refRobot, Robot* _object, Interaction* _interaction) {
  std::cout << "  SampleObjectPose " << _object->GetLabel() << " with refRobot " << _refRobot->GetLabel() << std::endl;

  auto library = this->GetMPLibrary();
  auto problem = _object->GetMPProblem();
  auto env = problem->GetEnvironment();
  auto objectCapability = _object->GetCapability();
  auto refRobotCapability = _refRobot->GetCapability();
  auto capability = objectCapability+"_"+refRobotCapability;

  std::cout << "   capability is: " << capability << std::endl;
...
// LidGraspStrategy::GroupCfgType
// LidGraspStrategy::
// SampleObjectPoses(RobotGroup* _group, Interaction* _interaction) {

//   std::cout << "SampleObjectPoses " << _group->GetLabel() << std::endl;
//   // Identify leader
...
//   std::cout << "____ computing ee frames: " << stages[_stage] << " ____" << std::endl;
...
//     for(auto role : f->GetRoles()) {
//       auto robot = m_roleMap[role]; // Objects
//       if(!robot->GetMultiBody()->IsPassive())
//         continue;
      
//       // mea existance check
//       if(robot->GetLabel().find("lid") == std::string::npos)
//         continue;
...
//       cfg.ConfigureRobot();
//       std::cout << " lid cfg: " << cfg.PrettyPrint() << std::endl;
...
//         } 
//         */
//         std::cout << "manipulator cfg: " << cfg.PrettyPrint() << std::endl;
//         return cfg;
//       }
//       else 
//         std::cout << "Invalid manipulator cfg: " << cfg.PrettyPrint() << std::endl;
//     }
...
//   if(_robot->GetLabel().find("ur5e") != std::string::npos) {
