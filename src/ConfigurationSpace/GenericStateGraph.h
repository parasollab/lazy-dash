...
#endif
{
  ...
  public:
    ...
    /// @param _source The source vertex.
    /// @param _target The target vertex.
    virtual bool IsChecked(const VID _source, const VID _target, const int _res) noexcept;

    /// Remove an edge from the graph if it exists.
    /// @param _iterator An iterator to the edge.
    virtual bool IsChecked(EI _iterator, const int _res) noexcept;

    virtual void SetChecked(const VID _source, const VID _target, const int _res) noexcept;

    /// Remove an edge from the graph if it exists.
    /// @param _iterator An iterator to the edge.
    virtual void SetChecked(EI _iterator, const int _res) noexcept;
...
AddVertex(const Vertex& _v) noexcept {
  ...
  // std::cout << "\nAdd Vertex: " << m_robot << " (" << vid << "): " << _v << std::endl;
...
AddEdge(const VID _source, const VID _target, const Edge& _w) noexcept {
  ...
  // std::cout << "\nAdd Edge: " << m_robot  << " ... " << _source << " --->> " << _target << std::endl;

  // std::cout << "\t " << this->GetVertex(_source) << std::endl;
  // std::cout << "\t " << this->GetVertex(_target) << std::endl;
  // std::cout << "\n" << std::endl;
...
DeleteEdge(const VID _source, const VID _target) noexcept {
  ...
  // std::cout << "\t\t\t\tDelete in GenericStateGraph " << m_robot << std::endl;
...
IsChecked(const VID _source, const VID _target, const int _res) noexcept {
  // Find the edge and crash if it isn't found.
  EID edgeDescriptor(_source, _target);
  VI dummy;
  EI edgeIterator;
...
IsChecked(EI _iterator, const int _res) noexcept {
  // auto check =  _iterator->property();
  // auto check2 =  _iterator->property().IsChecked(1);
...
SetChecked(EI _iterator, const int _res) noexcept {
  // _iterator->property().SetChecked(_res);
  auto check = _iterator->property();
  std::cout << check << std::endl;
}
