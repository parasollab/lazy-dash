...
class Hypergraph {
  ...
  public:
    ...
    virtual size_t AddHyperarc(std::set<size_t> _head, std::set<size_t> _tail,
                       HyperarcType _hyperarc, bool _overWrite=false, bool _checkExists=true);

    Hyperarc& GetHyperarc(size_t _hid);

    void SetHyperarcProperty(size_t _hid, HyperarcType _hyperarcType);
    ...
    void PrintGraphWithModes() const;
...
AddVertex(VertexType _vertex) {
  ...
  // std::cout << "Successfully added vertex: " << v.vid << std::endl;
...
AddHyperarc(std::set<size_t> _head, std::set<size_t> _tail,
            HyperarcType _hyperarc, bool _overWrite, bool _checkExists) {
  ...
  if(!_tail.empty() and _checkExists) {
    auto vid = *(_tail.begin());
    // std::cout << "check vid: " << vid << std::endl; 
    auto outgoingHIDs = GetOutgoingHyperarcs(vid);
    for(auto hid : outgoingHIDs) {
      auto hyperarc = GetHyperarc(hid);
      // std::cout << "L outgoingHIDs: " << hid << std::endl;
      // std::cout << "  L Head: " << hyperarc.head << " " << _head << std::endl;
      // std::cout << "  L Tail: " << hyperarc.tail << " " << _tail << std::endl;
      if(hyperarc.head == _head and hyperarc.tail == _tail) {
        // std::cout << "  They are the same" << std::endl;
        // Hyperarc already exists
        // Make sure there isn't a new property
        // std::cout << "  " << hyperarc.property << " " << _hyperarc << " ... overwrite? " << _overWrite <<std::endl; 
        if(hyperarc.property != _hyperarc) {
          if(_overWrite)
            ...
            throw RunTimeException(WHERE) << "Tried to add a different value for the hyperarc: "
                                        << hyperarc.hid << " (tail: " << hyperarc.tail << ", head: " << hyperarc.head << ")" << std::endl;
        }
        return hyperarc.hid;
      }
    }
...
void
Hypergraph<VertexType,HyperarcType>::
SetHyperarcProperty(size_t _hid, HyperarcType _hyperarcType) {
  m_hyperarcMap[_hid].property = _hyperarcType;
}
...
Print() const {
  ...
  std::cout << "HyperarcMap size: " << m_hyperarcMap.size() << std::endl;
...
PrintGraphWithModes() const {
  ...
  for(auto kv : m_hyperarcMap) {
    ...
    for(auto iter = arc.tail.begin(); iter != arc.tail.end(); iter++) {
      auto v = m_vertexMap.find(*iter);
      auto it = v->second.property.first;
      if (it) {
        auto group = it->GetGroup();
        std::cout << *iter << "(" << group->GetLabel() << ")";
      } else {
        std::cout << *iter << "(...)";
      }
      auto next = iter;
...
PrintModes() const {
  for (auto iter = m_vertexMap.begin(); iter != m_vertexMap.end(); iter++) {
    auto vid = iter->first;
    try {
      auto v = m_vertexMap.find(vid);
      auto it = v->second.property.first;
      
      if (it) {
        auto group = it->GetGroup();
        std::cout << vid << ": " << group->GetLabel() << std::endl;
      }
      else {
        std::cout << vid << ": not assigned " << std::endl;
      }
