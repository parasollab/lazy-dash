...
operator!=(const Formation& _formation) {
  ...
  if(m_robots.size() != _formation.m_robots.size()) {
    // std::cout << "robot size not matched: " << m_robots.size() << " " << _formation.m_robots.size() << std::endl;
    return true;
  }
  ...
  for(auto r1 : m_robots) {
    ...
    for(auto r2 : _formation.m_robots) {
      // std::cout << r1 << "(" << r1->GetLabel() << ") " << r2 << "(" << r2->GetLabel() << ")" << std::endl;
      if(r1 == r2) {
        // std::cout << "robots are the same" << std::endl;
        match = true;
        break;
      }
    }
  ...
  if(m_leader != _formation.m_leader) {
    // std::cout << "leaders are not the same: " << m_leader->GetLabel() << " " << _formation.m_leader->GetLabel() << std::endl;
    return true;
  }

  if(m_constraintMap != _formation.m_constraintMap) {
    // std::cout << "size compare: " << m_constraintMap.size() << " " << _formation.m_constraintMap.size() << std::endl;
    // std::cout << "formation1 info: " << std::endl;
    // for(auto kv : m_constraintMap) {
    //   auto mb = kv.first;
    //   std::cout << mb << std::endl; 
    // }
    // std::cout << "formation2 info: " << std::endl;
    // for(auto kv : _formation.m_constraintMap) {
    //   auto mb = kv.first;
    //   std::cout << mb << std::endl; 
    // }
    // std::cout << "constraintMap are not the same: " << m_leader->GetLabel() << " " << _formation.m_leader->GetLabel() << std::endl;
    return true;
  }
  // else {
    // std::cout << "size compare: " << m_constraintMap.size() << " " << _formation.m_constraintMap.size() << std::endl;
    ...
    //   std::cout << mb << std::endl; 
    // }
    // std::cout << "(not desired) constraintMap are the same: " << m_leader->GetLabel() << " " << _formation.m_leader->GetLabel() << std::endl;
  // }
...
ConvertToFormationDOF(std::vector<Cfg> _cfgs) {
  ...
  for(auto cfg : _cfgs) {
    ...
    size_t counter = dofCounterOffset;
    for(const auto& joint : mb->GetJoints()) {
      ...
      for(auto pair : indices) {
        ...
        // std::cout << "index: " << index << " counter: " << counter << " value: " << cfg[counter] << std::endl;
  ...
  for(size_t i = 0; i < m_multibody.GetJoints().size(); i++) {
    ...
    // std::cout << "index counter: " << counter << " value: " << value << std::endl;  
    dofs[counter+dofCounterOffset] = value;

    counter++;
  }
  
  // std::cout << "output dofs: " ;
  // for(auto e : dofs) {
  //   std::cout << e << " ";
  // }
  // std::cout << std::endl;
