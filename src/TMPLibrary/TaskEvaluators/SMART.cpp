...
SelectVertex(size_t _modeID, size_t _historyID, Mode _heuristic) {
  ...
  if(m_debug) {
    std::cout << "Target vertex: " << std::endl;
    for(auto kv : random) {
      std::cout << "\t" << kv.first->GetGroup()->GetLabel() << ": " << kv.second.PrettyPrint() << std::endl;
    }
    std::cout << "QNear: " << std::endl;
    
    auto aeVertex = m_actionExtendedGraph->GetVertex(closest);
    auto vertex = m_tensorProductRoadmap->GetVertex(aeVertex.vid);
    ...
    for(auto pair : vertex.cfgs) {
      ...
      // Compute distance to random sample
      auto rm = pair.first;
      auto gcfg = rm->GetVertex(pair.second);
      std::cout << "\t" << rm->GetGroup()->GetLabel() << ": " << gcfg.PrettyPrint() << std::endl;
    }
...
Extend(size_t _qNear, Direction _direction, size_t _modeID, 
  ...
  if(DRand() <= m_heuristicProb) {
    ...
    if(m_debug) {
      std::cout << "Heuristic extension direction:" << std::endl;
      for(auto kv : _direction) {
        std::cout << "\t" << kv.first->GetGroup()->GetLabel() << ": " << kv.second.PrettyPrint() << std::endl;
      }
    }
  }
  ...
  NeighborCandidates neighborCandidates;
  ...
  for(auto pair : vertex.cfgs) {
    ...
    auto& candidates = neighborCandidates[grm];
    ...
    if(passive) { 
      ...
      //auto cand = std::make_pair(0.,vid);
      candidates.emplace(0.,vid);
      continue;
    }
    ...
    // Add the staying put option
    candidates.emplace(minAngle,vid);
    ...
    for(auto eit = vit->begin(); eit != vit->end(); eit++) {
      ...
      candidates.emplace(angle,target);
  ...
  Vertex v;
  v.modeID = vertex.modeID;
  std::vector<Vertex> partials = {v};
  return ExtendToNeighbor(neighborCandidates, neighborCandidates.begin(), partials, _qNear); 
  ...
  if(m_distanceMap.find(aeSource) == m_distanceMap.end()) {
    std::cout << _qNear << ", " << qNear << ", " << _historyID << std::endl;
    state.vid = qNear;
    std::cout << m_actionExtendedGraph->GetVID(state) << std::endl;
    throw RunTimeException(WHERE) << "No distance map value for " << aeSource << std::endl;
  }

  m_distanceMap[aeTarget] = m_distanceMap[aeSource] + aeEdge.cost;

  if(m_debug) {
    std::cout << "EXTEND" << _qNear << ":" << qNear << " to " << aeTarget << ":" << qNew << std::endl;
    std::cout << m_distanceMap[aeTarget] << ", " << m_distanceMap[aeSource] << ", " << aeEdge.cost << std::endl;
  }
...
SMART::VID
SMART::
ExtendToNeighbor(NeighborCandidates& _neighborCandidates, NeighborCandidates::iterator _iter, std::vector<Vertex> _partials, const VID& _qNear) {

  if(_iter == _neighborCandidates.end()) {
    auto aeState = m_actionExtendedGraph->GetVertex(_qNear);
    auto qNear = aeState.vid;
    auto source = m_tensorProductRoadmap->GetVertex(qNear);

    for(auto vertex : _partials) {

      if(m_tensorProductRoadmap->IsVertex(vertex))
        return SIZE_MAX;

      auto cost = ValidConnection(source,vertex);
      if(cost == SIZE_MAX)
        return SIZE_MAX;

      // Add vertex to tensor product roadmap
      auto qNew = m_tensorProductRoadmap->AddVertex(vertex);
      Edge edge;
      //edge.transitions = transitions;
      edge.cost = double(cost);
      ...
      // Add to action extended graph
      ActionExtendedState state = aeState;
      //state.vid = qNear;
      //state.ahid = _historyID;
      ...
      m_historyVIDs[state.ahid].insert(aeTarget);
      m_historyVIDBias[state.ahid] = aeTarget;

      if(m_distanceMap.find(aeSource) == m_distanceMap.end()) {
        std::cout << _qNear << ", " << qNear << ", " << state.ahid << std::endl;
        state.vid = qNear;
        std::cout << m_actionExtendedGraph->GetVID(state) << std::endl;
        throw RunTimeException(WHERE) << "No distance map value for " << aeSource << std::endl;
      }
  ...
  // Create new partial vertices
  std::vector<Vertex> newPartials;
  auto iterNext = _iter;
  iterNext++;
  for(auto partial : _partials) {
    auto candidates = _iter->second;
    while(!candidates.empty()) {
      auto n = candidates.top();
      candidates.pop();
      auto newPartial = partial;
      newPartial.cfgs.emplace_back(_iter->first,n.second);
      newPartials.push_back(partial);
      auto newVID = ExtendToNeighbor(_neighborCandidates,iterNext,{newPartial},_qNear);
      if(newVID != SIZE_MAX)
        return newVID;
    }
  }
...
Rewire(size_t _qNew, size_t _qNear, size_t _modeID, size_t _historyID) {
  ...
  for(auto candidate : candidates) {
    ...
    for(auto pair1 : vertex1.cfgs) {
      ...
      for(auto pair2 : vertex2.cfgs) {
        ...
        auto edge = grm->GetEdge(source,target);
        //distance += dm->Distance(grm->GetVertex(source),grm->GetVertex(target));
        //distance += double(edge.GetTimeSteps());
        distance = std::max(distance,double(edge.GetTimeSteps()));
      }
  ...
  for(auto pair : neighbors) {
    ...
    if(m_debug) {
      std::cout << "REWIRING " << _qNew << " from " << pair.second << std::endl;
    }
    ...
    if(m_debug) {
      std::cout << "Rewiring edge with new cost: " << edge.cost << std::endl;
  ...
  for(auto pair : backNeighbors) {
    ...
    if(distance + m_distanceMap[_qNew] >= m_distanceMap[cand])
      continue;
    ...
    ActionExtendedEdge edge;
    edge.cost = m_distanceMap[_qNew] + distance;
    if(m_debug) {
      std::cout << "REWIRING " << _qNew << " to " << pair.second << std::endl;
      std::cout << "Rewiring edge with new cost: " << edge.cost << std::endl;
      std::cout << "Computed cost: " << cost << std::endl;
    }
...
ValidConnection(const Vertex& _source, const Vertex& _target) {
  ...
  for(auto kv : edges) {
    ...
    if(grm->IsEdge(source,target)) {
      auto edge = grm->GetEdge(source,target);
      debugTimesteps = std::max(debugTimesteps,edge.GetTimeSteps());
    }
  }

  if(m_debug) {
    std::cout << "Current timesteps: " << maxTimestep << ". Debug timesteps: " << debugTimesteps << std::endl;
  }
        ...
        for(auto r1 : group1->GetRobots()) {
          for(auto r2 : group2->GetRobots()) {
            ...
            if(cd->IsMultiBodyCollision(cdInfo,
              ...
              if(m_debug) {
                std::cout << "Failed extension because of collision between " << r1->GetLabel() << " and " << r2->GetLabel() << std::endl;
              }
              return SIZE_MAX;
            }
          }
...
CheckForModeSwitch(size_t _qNew) {
  ...
  for(auto modeSwitch : modeSwitches) {
    ...
    if(m_debug) {
      ...
      std::cout << _qNew << " to " << aeVID << ", " << aeEdge.cost << std::endl;
    }
