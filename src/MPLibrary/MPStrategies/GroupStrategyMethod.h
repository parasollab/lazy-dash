...
class GroupStrategyMethod : public MPStrategyMethod<MPTraits> {
  ...
  protected:
    ...
    /// Get a map from robot -> boundary for the current group tasks's path
    /// constraints.
    // TODO::Currently assumes only a single path constraint for any robot.
    BoundaryMap GetPathBoundaryMap() const noexcept;
...
typename GroupStrategyMethod<MPTraits>::BoundaryMap
GroupStrategyMethod<MPTraits>::
GetPathBoundaryMap() const noexcept {
  const Boundary* const boundary = this->GetEnvironment()->GetBoundary();
  BoundaryMap pathConstraintMap;
  if(this->m_modeSampling) {
