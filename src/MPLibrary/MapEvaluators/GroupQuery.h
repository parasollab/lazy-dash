...
class GroupQuery: public MapEvaluatorMethod<MPTraits> {
  ...
  protected:
    ...
    virtual double StaticPathWeight(typename GroupRoadmapType::adj_edge_iterator& _ei,
        const double _sourceDistance, const double _targetDistance) const;

    virtual double DynamicPathWeight(typename GroupRoadmapType::adj_edge_iterator& _ei,
        const double _sourceDistance, const double _targetDistance) const;
...
operator()() {
  if(this->m_debug)
    std::cout << "GroupQuery OPERATOR" << std::endl;
  ...
  for(; m_goalIndex < numGoals; ++m_goalIndex) {
    ...
    if(start == MAX_UINT)
      throw RunTimeException(WHERE) << "2 No VIDs located for start.";
