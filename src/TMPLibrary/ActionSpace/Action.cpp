...
const bool
Action::
IsDirectReverseAllowed() const {
  return m_isDirectReverseAllowed;
}

void
Action::
SetDirectReverseAllowed(bool _isDirectReverseAllowed) {
  m_isDirectReverseAllowed = _isDirectReverseAllowed;
}
...
ParseXMLNode(XMLNode& _node) {
  ...
  m_isTransfer = _node.Read("transfer",false,m_isTransfer,
        "Flag indiciating if this action is transfering object to the other task space.");

  // m_isDirectReverseAllowed = _node.Read("allowDirectReverse",false,m_isDirectReverseAllowed,
  //       "Flag indicating if this action is allowed to direct reverse");
