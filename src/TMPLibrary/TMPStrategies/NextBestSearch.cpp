...
PlanTasks() {
  ...
  bool extraSolved = false;

  while(true) {

    if(m_extraEval != "" and !extraSolved) {
      ...
      while(!foundSolution) {
        ...
        if(me2->operator()(plan)) {
          std::cout << "me plan valid" << std::endl;
          if(plan->GetCost() < lb) {
    ...
    std::cout << "upperBound and lowerBound " << m_upperBound <<" " << lowerBound << std::endl;
    ...
    if(m_upperBound < MAX_DBL)
      break;
    std::cout << "EXPANDING REPRESENTATION" << std::endl;
    plan->SetDecomposition(originalDecomp);
    std::cout << "Regenerating Representation" << std::endl;
    mg->GenerateRepresentation();
    // lowerBound = 0;
...
SaveSolution(const Node& _node) {
    ...
    for(auto iter1 = cfgPaths.begin(); iter1 != cfgPaths.end(); iter1++) {
      ...
      for(auto robot : group1->GetRobots()) {
        ...
        if(robot->GetLabel().find("ur5e") != std::string::npos) {
          if(robot->GetLabel().find("mobile") != std::string::npos) {
            gripperIdx = 4;
          }
          else {
            gripperIdx = 1;
        ...
        std::cout << "gripper idx: " << gripperIdx << std::endl;
  ...
  for(size_t t = 1; t < finalTime; t++) {
    for(auto& kv : robotPaths) {
      ...
      else if(kv.first->GetLabel().find("gantry") != std::string::npos) {
        gripperIdx = 0;
      }
      std::cout << "gripper idx: " << gripperIdx << std::endl;
      
      if(abs(cfg1[gripperIdx] - cfg2[gripperIdx]) >= .00001) {
