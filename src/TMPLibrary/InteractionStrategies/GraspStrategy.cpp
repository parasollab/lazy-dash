...
operator()(Interaction* _interaction, State& _start) {
  ...
  for(size_t i = 1; i < graspStage; i++) {
    ...
    */
    ...
    for(auto kv : eeFrames) {
      ...
      if(!cfg.GetRobot()) {
        if(m_debug) {
          std::cout << "1 Failed to find a valid grasp pose for " << kv.first->GetLabel();
        }
        m_roleMap.clear();
        // Set all robots to virtual
        for(auto kv : coord->GetInitialRobotGroups()) {
  ...
  */
  ...
  for(auto kv : nextStageEEFrames) {
    ...
    if(!cfg.GetRobot()) {
      if(m_debug) {
        std::cout << "2 Failed to find a valid grasp pose for " << kv.first->GetLabel();
      }
      m_roleMap.clear();
      // Set all uninvolved robots back to non virtual
      for(auto& robot : problem->GetRobots()) {
  ...
  */
  ...
  for(int i = 0; i < num_sols; i++) {
    ...
    if(vc->IsValid(cfg,this->GetNameAndLabel())) {
      ...
      */
      if(m_debug)
        std::cout << cfg.PrettyPrint() << " valid." << std::endl;
      return cfg;
    }
    
    if(m_debug) 
      std::cout << cfg.PrettyPrint() << " invalid." << std::endl;
  }
