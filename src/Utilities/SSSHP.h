...
template <typename VertexType, typename HyperarcType>
using SSSHPForwardStarWithConstraints =
  std::function<std::set<size_t>(
    const size_t _vid,
    Hypergraph<VertexType,HyperarcType>* _h,
    std::set<size_t> _restrictedHIDs
  )>;


template <typename VertexType, typename HyperarcType>
SSSHPForwardStarWithConstraints<VertexType,HyperarcType>
DefaultSSSHPForwardStarWithConstraints() {
  return [](const size_t& _vid, Hypergraph<VertexType,HyperarcType>* _h, std::set<size_t> _restrictedHIDs) {
    return _h->GetOutgoingHyperarcs(_vid);
  };
}
...
SBTDijkstra(
  ...
  auto output = SBTDijkstraWithConstraints(_h,_source,_weight,
...
SBTDijkstra(
  ...
  SSSHPPathWeightFunction<VertexType,HyperarcType> _weight,
  SSSHPTerminationCriterion _termination,
  SSSHPForwardStarWithConstraints<VertexType,HyperarcType> _forwardStar,
  SSSHPHeuristic<VertexType,HyperarcType> _heuristic,
  std::set<size_t> _restrictedHIDs,
  const double _sourceWeight = 0) {
  ...
  auto output = SBTDijkstraWithConstraints(_h,_source,_weight,
                                        _termination,_forwardStar,
                                        _heuristic,_restrictedHIDs,_sourceWeight);
  return output.first;
}
...
SBTDijkstraWithConstraints(
  ...
  SBTDijkstra(_h,mbt,pq,_weight,_termination,_forwardStar,_heuristic,_restrictedHIDs);
...
SBTDijkstra(
  ...
  while(!_pq.empty()) {
    ...
    // Iterate through the forward start of the current vertex.
    for(auto hid : _forwardStar(current.vid,_h,_restrictedHIDs)) {
