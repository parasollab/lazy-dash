...
class GroupPath final {
  ...
  public:
    ...
    std::vector<std::pair<size_t,size_t>> GetIntervals() const;

    std::vector<size_t> GetDurations() const;
    ...
    std::pair<size_t,size_t> GetEdgeAtTimestepInIndices(size_t _timestep);
  ...
  private:
    ...
    mutable std::vector<std::pair<size_t,size_t>> m_intervals;
    mutable bool m_intervalCached{false};

    mutable std::vector<size_t> m_durations;
    mutable bool m_durationCached{false};
    ///@}
};
...
PrintIntervals() const {
  ...
  for(auto start = m_vids.begin(); start + 1 < m_vids.end(); ++start) {
    ...
    // Add this edge's weight to the sum.
    const auto& edge = m_roadmap->GetEdge(*start, *(start + 1));
    std::cout << *start << " --> " << *(start+1) << ": " << edge.GetTimeSteps() << std::endl;
  }
}
...
GetIntervals() const {
  ...
  for(auto start = m_vids.begin(); start + 1 < m_vids.end(); ++start) {
    ...
    const auto& edge = m_roadmap->GetEdge(*start, *(start + 1));
    m_intervals.push_back(std::make_pair(curr,curr+edge.GetTimeSteps()-1));
    curr += edge.GetTimeSteps();
  }
  m_intervals.push_back(std::make_pair(curr,curr));
  return m_intervals;
}
...
GetDurations() const {
  ...
  for(auto start = m_vids.begin(); start + 1 < m_vids.end(); ++start) {
    ...
    const auto& edge = m_roadmap->GetEdge(*start, *(start + 1));
    m_durations.push_back(edge.GetTimeSteps());
  }
  m_durations.push_back(0);
  return m_durations;
}
...
SetWaitTimes(std::vector<size_t> _waitTimes) {
  ...
  if(m_waitingTimesteps.size() != m_vids.size()) {
    throw RunTimeException(WHERE) << "Size of timestep vector does not "
                                  << "match path length"
                                  << "waiting timesteps: " << m_waitingTimesteps
                                  << " vids timeseepts: " << m_vids;
  }
}
...
std::pair<size_t,size_t>
GroupPath<MPTraits>::
GetEdgeAtTimestepInIndices(size_t _timestep) {
  size_t step = 0;
  for(size_t i = 0; i + 1 < m_vids.size(); i++) {
    if(!m_waitingTimesteps.empty())
      step += m_waitingTimesteps[i];
  ...
  return std::make_pair(m_vids.size()-1,m_vids.size()-1);
}
