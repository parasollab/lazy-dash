  ...
  : BoundaryConstraint(_r, nullptr)
{
  ...
  const std::string pointString = _node.Read("point", false, "", "The Cfg point"),
                    ...
                    label = _node.Read("label", false, "", "The label of Cspace");

  m_constraint_label = label;

  std::cout << "L point string read from xml: " << pointString << std::endl;
  std::cout << "L bbx string read from xml: " << bbxString << std::endl;
  std::cout << "L Label read from xml: " << this->GetLabel() << std::endl;
