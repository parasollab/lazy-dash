...
operator()() {
  if(this->m_debug)
    std::cout << "SIPP OPERATOR" << std::endl;
  Reset();
  ...
  const size_t numGoals = this->GetGroupTask() ? this->GetGroupTask()->GetNumGoals()
    ...
    if(this->m_debug) {
      std::cout << "Start: " << goalTracker->GetStartVIDs() << std::endl;
      std::cout << "Num Goals: " << numGoals << std::endl;
      std::cout << "Unreached Goal Indices: ";
      for(auto idx : unreachedGoals) {
        std::cout << idx << " ";
      }
      std::cout << std::endl;
    }
  ...
  for(; m_goalIndex < numGoals; ++m_goalIndex) {
    ...
    if(m_startVID != MAX_UINT) {
      ...
      for(auto vid : goalTracker->GetStartVIDs()) {
        if(SameFormations(r->GetActiveFormations(), r->GetVertex(vid).GetFormations())) {
          start = vid;
          break;
        }
      }
    ...
    if(start == MAX_UINT){
      throw RunTimeException(WHERE) << "3 No VIDs located for start.";
    }
    ...
    if(m_startVID == MAX_UINT) {
      ...
      if(this->m_debug) 
        std::cout << "Add sipp vertex " << vertex.vid << std::endl;
...
PerformSubQuery(const size_t _start, const std::vector<size_t> _goals) {
  if(this->m_debug)
    std::cout << "SIPP SubQuery " << std::endl;
...
PathWeight(typename SIPPGraph::adj_edge_iterator& _ei,
  ...
  for(auto iter = intervals.begin(); iter != intervals.end(); iter++) {
    ...
    if(minWaitTime > 0)
      minWaitTime += m_buffer;
    const size_t minStartTime = sourceDistance + minWaitTime;

    // Check source interval for min start time
    if(!source.interval.Contains(std::max(std::min(minStartTime-m_buffer,minStartTime),m_startTime)) 
    or !source.interval.Contains(std::max(minStartTime,minStartTime+m_buffer))) {
      continue;
    }

    // Check edge interval for min start time
    if(!interval.Contains(std::max(std::min(minStartTime-m_buffer,minStartTime),m_startTime)) 
    or !interval.Contains(std::max(minStartTime,minStartTime+m_buffer))) {
      continue;
    }
    ...
    if(!target.interval.Contains(std::max(std::min(minEndTime-m_buffer,minEndTime),m_startTime)) 
    or !target.interval.Contains(std::max(minEndTime+m_buffer,minEndTime))) {
      // Check that min end time is not greater than the interval range
      if(target.interval.max < std::max(minEndTime+m_buffer,minEndTime)) {
        continue;
      }
      ...
      if(!source.interval.Contains(std::max(std::min(startTime-m_buffer,startTime),m_startTime)) 
      or !source.interval.Contains(std::max(startTime+m_buffer,startTime))) {
        continue;
      }

      // Check edge interval for start time
      if(!interval.Contains(std::max(std::min(startTime-m_buffer,startTime),m_startTime)) 
      or !interval.Contains(std::max(startTime+m_buffer,startTime))) {
        continue;
      }

      if(!target.interval.Contains(std::max(std::min(startTime+duration-m_buffer,startTime+duration),m_startTime)) 
      or !target.interval.Contains(std::max(startTime+duration+m_buffer,startTime+duration)))
        continue;
...
BuildNeighbors(typename SIPPGraph::vertex_descriptor _sippSource, size_t _rmTarget,
  ...
  for(auto& inter : endIntervals) {
    ...
    if(this->m_debug)
      std::cout << "Add sipp neighbor vertex " << targetVertex.vid << std::endl;
