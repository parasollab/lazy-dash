...
ConstructObjectRoadmap(Robot* _object) {
  ...
  // Set all other robots virtual
  for(const auto& r : problem->GetRobots()) {
    if(r.get() == _object)
      continue;
  ...
  // Return robots virtual to non-virtual
  for(const auto& r : problem->GetRobots()) {
    if(r.get() == plan->GetCoordinator()->GetRobot())
      continue;
  ...
  for(auto f : formations) {
    ...
    for(auto robot : group->GetRobots()) {
      ...
      if(!robot->GetMultiBody()->IsPassive()) {
        ...
        auto pc = std::unique_ptr<BoundaryConstraint>(new BoundaryConstraint(robot,std::move(bbx)));
        mt.AddPathConstraint(std::move(pc));

        auto sc = std::unique_ptr<CSpaceConstraint>(new CSpaceConstraint(robot,problem->GetInitialCfg(robot)));
        mt.SetStartConstraint(std::move(sc));
      }
      else {
        auto sc = std::unique_ptr<BoundaryConstraint>(new BoundaryConstraint(robot,std::move(problem->GetEnvironment()->GetBoundary()->Clone())));
        mt.SetStartConstraint(std::move(sc));
      }
...
SampleInteractionWithPassive(Interaction* _interaction, State _state, RobotGroup* _passive) {
  ...
  for(auto vit = rm->begin(); vit != rm->end(); vit++) {
    ...
    for(size_t i = 0; i < m_interactionAttempts; i++) {
      if(RunInteractionStrategy(_interaction,_state)) {
        localSuccess = true;
        break;
      }
...
IsReachable(const Terrain* _terrain, Robot* _robot) {
  ...
  Vector3d center1 = mb->GetBase()->GetWorldTransformation().translation();
...
IsReachable(Robot* _robot1, Robot* _robot2) {
  ...
  Vector3d center1 = mb1->GetBase()->GetWorldTransformation().translation();
  Vector3d center2 = mb2->GetBase()->GetWorldTransformation().translation();
