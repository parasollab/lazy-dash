...
RecordStrategy::
RecordStrategy() {
  this->SetName("RecordStrategy");
}

RecordStrategy::
RecordStrategy(XMLNode& _node) : GraspStrategy(_node) {
  this->SetName("RecordStrategy");
...
operator()(Interaction* _interaction, State& _start) {
  ...
  // // Sort out the irrelevant objects
  // std::vector<RobotGroup*> overlapPointer;
  // for(auto it = tailModeMap.begin(); it != tailModeMap.end(); ++it){
  //   for(auto it2 = headModeMap.begin(); it2 != headModeMap.end(); ++it2){
  //     if(it->first==it2->first)
  //       overlapPointer.push_back(it->first);
  //   }
  // }
  // // Remove the collected pointers from tailModeMap
  // for (const auto& pointer : overlapPointer) {
  //   tailModeMap.erase(pointer);
  // }
  // // Remove the collected pointers from headModeMap
  // for (const auto& pointer : overlapPointer) {
  //   headModeMap.erase(pointer);
  // }
  ...
  for(auto kv : _start) {
    ...
    if(active){
      ...
      auto transform = passive->GetMultiBody()->GetBase()->GetWorldTransformation();
      ...
      objectPose.ConfigureRobot();
      std::cout << "object escape pose: " << objectPose.PrettyPrint() << std::endl;
    }
    else{
  ...
  for(auto kv : _start) {
    ...
    if(active) {
      Cfg cfg = objectPoses[passive];
      gcfg.GetRandomGroupCfg(constraintMap[active]->GetBoundary());
      gcfg.SetRobotCfg(passive,std::move(cfg));
...
//       // mea existance check
//       if(robot->GetLabel().find("mea") == std::string::npos)
//         continue;
...
//       cfg.ConfigureRobot();
//       std::cout << " mea cfg: " << cfg.PrettyPrint() << std::endl;
