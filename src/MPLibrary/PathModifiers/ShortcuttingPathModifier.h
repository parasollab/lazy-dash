#ifndef SHORT_CUTTING_PATH_MODIFIER_H_
#define SHORT_CUTTING_PATH_MODIFIER_H_

#include "PathModifierMethod.h"

////////////////////////////////////////////////////////////////////////////////
/// @ingroup PathModifiers
/// @brief TODO.
///
/// TODO.
////////////////////////////////////////////////////////////////////////////////
template <typename MPTraits>
class ShortcuttingPathModifier : public PathModifierMethod<MPTraits> {

  public:

    typedef typename MPTraits::CfgType            CfgType;
    typedef typename MPTraits::RoadmapType        RoadmapType;
    typedef typename RoadmapType::VID             VID;

    typedef typename MPTraits::GroupCfgType       GroupCfgType;
    typedef typename MPTraits::GroupRoadmapType   GroupRoadmapType;

    ShortcuttingPathModifier(const string& _dmLabel = "",
        const string& _lpLabel = "");
    ShortcuttingPathModifier(XMLNode& _node);

    virtual void ParseXML(XMLNode& _node);
    virtual void Print(ostream& _os) const;

    bool ModifyImpl(RoadmapType* _graph, vector<CfgType>& _path, vector<CfgType>& _newPath) override;

    bool ModifyImpl(GroupRoadmapType* _graph, vector<GroupCfgType>& _path,
        vector<GroupCfgType>& _newPath) override;

    std::vector<std::pair<size_t,size_t>> ArrivalTimes(vector<GroupCfgType>&
        _path);

    void PathCost(vector<CfgType>& _path);
    void GroupPathCosts(vector<GroupCfgType>& _path);
    void PrintPath(vector<CfgType>& _path);
    void PrintGroupPath(vector<GroupCfgType>& _path);

  private:
    string m_lpLabel; // Local planner
};

template <typename MPTraits>
ShortcuttingPathModifier<MPTraits>::
ShortcuttingPathModifier(const string& _dmLabel, const string& _lpLabel) :
  PathModifierMethod<MPTraits>(), m_lpLabel(_lpLabel) {
    this->SetName("ShortcuttingPathModifier");
  }

template <typename MPTraits>
ShortcuttingPathModifier<MPTraits>::
ShortcuttingPathModifier(XMLNode& _node) :
  PathModifierMethod<MPTraits>(_node) {
    this->SetName("ShortcuttingPathModifier");
    ParseXML(_node);
  }

template <typename MPTraits>
void
ShortcuttingPathModifier<MPTraits>::
ParseXML(XMLNode& _node) {
  m_lpLabel = _node.Read("lpLabel", true, "", "Local planner method");
}

template <typename MPTraits>
void
ShortcuttingPathModifier<MPTraits>::
Print(ostream& _os) const {
  PathModifierMethod<MPTraits>::Print(_os);
  _os << "\tlocal planner = \"" << m_lpLabel << "\"" << endl;
}

// Shortens the path by skipping nodes with a greedy approach.
// This function is also supposed to be used before MedialAxisSmooth.
template <typename MPTraits>
bool
ShortcuttingPathModifier<MPTraits>::
ModifyImpl(RoadmapType* _graph, vector<CfgType>& _path, vector<CfgType>& _newPath) {
// Shortens the path by skipping nodes with a greedy approach.
// This function is also supposed to be used before MedialAxisSmooth.

  if(_path.empty())
    throw RunTimeException(WHERE) << "Path is empty.";

  auto lp = this->GetLocalPlanner(m_lpLabel);
  Environment* env = this->GetEnvironment();
  StatClass* stats = this->GetStatClass();
  LPOutput<MPTraits> tmpOutput;
  double posRes = env->GetPositionRes();
  double oriRes = env->GetOrientationRes();
  stats->StartClock("Path Modifier");

  std::vector<CfgType> shortcutPoints;
  // Make room for the path
  _newPath.clear();

  //Save the first cfg in the path, it can never be skipped.
  shortcutPoints.push_back(_path[0]);

  size_t n = _path.size();
  size_t i = 0;   // i is index our source point
  size_t j = n-1; //j is index of the attempet target to shortcut
  bool skip;

  while(i < j) {
    //Try to make a connection by skipping intermediates betwen i and j
    skip = lp->IsConnected(_path[i],_path[j], &tmpOutput,
        posRes, oriRes, true, true);
    if(skip) {
      shortcutPoints.push_back(_path[j]);
      i = j;
      j = n-1;
    }
    else {
      //Could not skip. Try the previous node.
      --j;
    }
  }
  if(shortcutPoints.empty())
    return false;
  else {
    // If we have a path (in terms of shortcutPoints) we have to reconstruct it,
    // part of this function is grabbed from Path::FullCfgs, we may do this ina
    // better way.

    // Insert the first vertex.
    std::vector<CfgType> out = {shortcutPoints[0]};

    for(size_t i = 1 ; i < shortcutPoints.size(); ++i) {
      // Insert intermediates between vertices.
      std::vector<CfgType> edge =
        this->GetMPLibrary()->ReconstructEdge(shortcutPoints[i-1],
            shortcutPoints[i], posRes, oriRes);
      out.insert(out.end(), edge.begin(), edge.end());

      //Insert the next vertex.
      out.push_back(shortcutPoints[i]);
    }

    _newPath = out;
  }
  PathCost(_newPath);
  PrintPath(_newPath);
  return true;
}

template <typename MPTraits>
bool
ShortcuttingPathModifier<MPTraits>::
ModifyImpl(GroupRoadmapType* _graph, vector<GroupCfgType>& _path, vector<GroupCfgType>& _newPath) {
// Shortens the path by skipping nodes with a greedy approach.
// This function is also supposed to be used before MedialAxisSmooth.

  if(_path.empty())
    throw RunTimeException(WHERE) << "Path is empty.";
  
  auto lp = this->GetLocalPlanner(m_lpLabel);
  Environment* env = this->GetEnvironment();
  // StatClass* stats = this->GetStatClass();
  GroupLPOutput<MPTraits> tmpOutput(this->GetGroupRoadmap());
  double posRes = env->GetPositionRes();
  double oriRes = env->GetOrientationRes();
  // stats->StartClock("Path Modifier");

  std::vector<GroupCfgType> shortcutPoints;
  // Make room for the path
  _newPath.clear();

  //Save the first cfg in the path, it can never be skipped.
  shortcutPoints.push_back(_path[0]);

  auto arrivalTimes = ArrivalTimes(_path);
  size_t n = 0;
  size_t goalCounter = 0;
  size_t i = 0;   // i is index our source point
  size_t j = 0; //j is index of the attempet target to shortcut

  //auto groupGoal = _path.back();
  //const auto& robots = groupGoal.GetRobots();
  //while(goalCounter <= robots.size()) {
  // std::cout << "Arrival Times: ";
  // for(auto pair : arrivalTimes) {
  //   std::cout << pair << std::endl;
  // }
  while(n < _path.size()-1) {
    n = arrivalTimes[goalCounter].second;
    j = n - 1;
    bool skip;
    std::cout << "\tArrival Times: " << arrivalTimes[goalCounter] << " (Goal Counter " << goalCounter << ")" << std::endl;

    while(i < j) {
      //Try to make a connection by skipping intermediates betwen i and j
      skip = lp->IsConnected(_path[i],_path[j], &tmpOutput,
          posRes, oriRes, true, true);
      std::cout << "(" << i << ") " << _path[i].PrettyPrint() << " --> " << "(" << j << ") " << _path[j].PrettyPrint() << " ";
      if(skip) {
        shortcutPoints.push_back(_path[j]);
        i = j;
        j = n-1;
        std::cout << ": valid" << std::endl;
      }
      else {
        //Could not skip. Try the previous node.
        --j;
        std::cout << ": invalid" << std::endl;
      }
    }
    i = n;
    ++goalCounter;
  }
  if(shortcutPoints.size() <= 1) {
    std::cout << "No valid path" << std::endl;
    return false;
  }
  else {
    std::cout << "c" << std::endl;
    // If we have a path (in terms of shortcutPoints) we have to reconstruct it,
    // part of this function is grabbed from Path::FullCfgs, we may do this ina
    // better way.

    // Insert the first vertex.
    std::vector<GroupCfgType> out = {shortcutPoints[0]};

    for(size_t i = 1 ; i < shortcutPoints.size(); ++i) {
      std::cout << "\tc.1" << std::endl;
      // Insert intermediates between vertices.
      std::vector<GroupCfgType> edge =
        this->GetMPLibrary()->ReconstructEdge(shortcutPoints[i-1],
            shortcutPoints[i], posRes, oriRes);
      out.insert(out.end(), edge.begin(), edge.end());
      std::cout << edge.size() << std::endl;
      //Insert the next vertex.
      out.push_back(shortcutPoints[i]);
    }
    _newPath = out;
  }
  std::cout << "d" << std::endl;

  GroupPathCosts(_newPath);
  PrintGroupPath(_newPath);
  // stats->StopClock("Path Modifier");
  return true;
}

template <typename MPTraits>
std::vector<std::pair<size_t,size_t>>
ShortcuttingPathModifier<MPTraits>::
ArrivalTimes(vector<GroupCfgType>& _path) {

  // We get all the RobotCfgs,
  auto groupGoal = _path.back();
  const auto& robots = groupGoal.GetRobots();
  std::vector<std::pair<size_t,size_t>> arrivalTimes; // <index,timestep>
  std::map<size_t,size_t> arrivalTimesMap; // <index,timestep>

  for(size_t j = 0 ; j < _path.size() ; ++j ) {
    for(size_t i = 0; i < robots.size(); ++i) {
      auto& goal = groupGoal.GetRobotCfg(i);
      if(goal == _path[j].GetRobotCfg(i)) {
        if(arrivalTimesMap.count(i))
          continue;
        arrivalTimesMap[i] = j;
        arrivalTimes.push_back(std::make_pair(i,j));
      }
    }
  }
  return arrivalTimes;
}

/*------------------------ Stats -----------------------------------*/

template <typename MPTraits>
void
ShortcuttingPathModifier<MPTraits>::
PathCost(vector<CfgType>& _path) {
  const double timeRes = this->GetEnvironment()->GetTimeRes();
  this->GetStatClass()->SetStat(this->GetNameAndLabel() + "::Cost", _path.size()*timeRes);

}

template <typename MPTraits>
void
ShortcuttingPathModifier<MPTraits>::
GroupPathCosts(vector<GroupCfgType>& _path) {

  // We get all the RobotCfgs,
  auto groupGoal = _path.back();
  const double timeRes = this->GetEnvironment()->GetTimeRes();
  const auto& robots = groupGoal.GetRobots();
  size_t sum_of_cost = 0;

  for(size_t i = 0; i < robots.size(); ++i) {
    auto& goal = groupGoal.GetRobotCfg(i);
    for(size_t j = 0 ; j < _path.size() ; ++j ) {
      if(goal == _path[j].GetRobotCfg(i))
        break;
      else
        ++sum_of_cost;
    }
  }
  auto stats = this->GetStatClass();
  stats->SetStat(this->GetNameAndLabel() + "::SumOfCost", sum_of_cost*timeRes);
  stats->SetStat(this->GetNameAndLabel() + "::Makespan", _path.size()*timeRes);
}

/*------------------------ PrintPaths -----------------------------------*/

template <typename MPTraits>
void
ShortcuttingPathModifier<MPTraits>::
PrintPath(vector<CfgType>& _path) {

  const std::string base = this->GetBaseFilename();
  std::string outPathFile = base + ".modified.path";
  WritePath(outPathFile, _path);
}


template <typename MPTraits>
void
ShortcuttingPathModifier<MPTraits>::
PrintGroupPath(vector<GroupCfgType>& _groupPath) {

  const std::string base = this->GetBaseFilename();
  std::string outPathFile = base + ".modified.path";
  WritePath(outPathFile, _groupPath);
}



/*
template <typename MPTraits>
bool
ShortcuttingPathModifier<MPTraits>::
ModifyImpl(RoadmapType* _graph, vector<CfgType>& _path, vector<CfgType>& _newPath) {
  if(this->m_debug) cout << "\n*S* Executing ShortcuttingPathModifier::Modifier" << endl;

  auto graph = _graph ? _graph : this->GetRoadmap();

  vector<VID> originalPathVIDs = this->GetPathVIDs(_path, graph);

  bool smoothFileOutput = false;

  if(!originalPathVIDs.empty()) {
    smoothFileOutput = true;

    auto lp = this->GetLocalPlanner(m_lpLabel);
    Environment* env = this->GetEnvironment();
    StatClass* stats = this->GetStatClass();
    LPOutput<MPTraits> tmpOutput;

    bool reconstruct, skip;
    double posRes = env->GetPositionRes();
    double oriRes = env->GetOrientationRes();

    stats->StartClock("Path Modifier");
    //This variable will store how many nodes were skipped
    size_t skips = 0;

    vector<VID> newPathVIDs;
    // Make room for the path
    _newPath.clear();

    //Save the first node in the path. (i.e. it can never be skipped)
    newPathVIDs.push_back(originalPathVIDs[0]);
    _newPath.push_back(graph->GetVertex(originalPathVIDs[0]));

    size_t n = originalPathVIDs.size();
    size_t i = 0;   //i is the index of the start VID (configuration)
    size_t j = n-1; //j is the index of the goal VID (configuration)
    while(i < j) {
      if((i+1) == j) {//If the vertices are adjacent, there are no nodes to skip
        //Only reconstruct and save path if smoothing output is wanted.
        if(smoothFileOutput) {
          reconstruct = lp->IsConnected(graph->GetVertex(originalPathVIDs[i]),
              graph->GetVertex(originalPathVIDs[j]), &tmpOutput,
              posRes, oriRes, true, true);
          this->AddToPath(_newPath, &tmpOutput,
              graph->GetVertex(originalPathVIDs[j]));
          //If could not reconstruct, abort the output,
          //but continue skipping nodes
          if(!reconstruct) {
            cerr << "*S* Error. Could not reconstruct path in ShortcuttingPathModifier::Modifier()" << endl;
            smoothFileOutput = false;
            if(_newPath.empty())
              cerr << "*S*\t_newPath is empty. Cannot write anything" << endl;
            else{
              cerr << "*S*\tWill output failed path to \"error.smooth.path\"" << endl;
              WritePath("error.smooth.path", _newPath);
              _newPath.clear();
            }
            return false;
          }
        }
        newPathVIDs.push_back(originalPathVIDs[j]);
        i = j;
        j = n-1;
      }
      else {
        //Try to make a connection by skipping nodes betwen i and j
        skip = lp->IsConnected(graph->GetVertex(originalPathVIDs[i]),
            graph->GetVertex(originalPathVIDs[j]), &tmpOutput,
            posRes, oriRes, true, true);
        if(skip) {
          if(smoothFileOutput)
            this->AddToPath(_newPath, &tmpOutput,
                graph->GetVertex(originalPathVIDs[j]));
          newPathVIDs.push_back(originalPathVIDs[j]);
          skips += (j-i-1);
          i = j;
          j = n-1;
        }
        else {
          //Could not skip. Try the previous node.
          --j;
        }
      }
    }

    if(this->m_debug) {
      if(skips == 0)
        cout << "*S* Could not skip any nodes" << endl;
      else{
        cout << "*S* Smoothing operation skipped " << skips;
        if(skips == 1)
          cout << " node" << endl;
        else
          cout << " nodes" << endl;
      }
    }

    stats->StopClock("Path Modifier");

    return true;
  }
  else{
    cerr << "*S* Error. originalPathVIDs in " << this->GetNameAndLabel()
      << " is empty. Aborting smoothing operation(s)." << endl;
    return false;
  }

}
*/


#endif