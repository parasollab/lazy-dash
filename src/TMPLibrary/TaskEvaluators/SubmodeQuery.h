...
class SubmodeQuery : public TaskEvaluatorMethod { 
  ...
  public:
    ...
    std::unordered_map<SemanticTask*,size_t> GetVertexTasks();

    std::unordered_map<SemanticTask*,size_t> GetHyperarcTasks();

    // std::set<std::set<SemanticTask*>> GetTaskSetMap();
    size_t GetTaskSetMap();
  ...
  protected:
    ...
    ActionExtendedHypergraph m_actionExtendedHypergraphLocal;
    ...
    std::set<std::set<SemanticTask*>> m_taskSets;
