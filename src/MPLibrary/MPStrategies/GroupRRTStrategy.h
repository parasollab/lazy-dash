...
class GroupRRTStrategy : public GroupStrategyMethod<MPTraits> {
  ...
  protected:
    ...
    virtual std::vector<CfgType> SelectFormationTarget(Formation* _formation);
...
GroupRRTStrategy(XMLNode& _node) : GroupStrategyMethod<MPTraits>(_node) {
  ...
  std::cout << "Construct GroupRRT" << std::endl;
...
GroupRRTStrategy<MPTraits>::
Initialize() {
  std::cout << "Initialize GroupRRT" << std::endl;
...
SelectIndividualTarget() {
  ...
  if(this->m_debug) {
    std::cout << "path constraints size: " << this->GetTask()->GetPathConstraints().size() << std::endl;
    std::cout << "mode sampling: " << this->m_modeSampling << std::endl;
  }
  const Boundary* samplingBoundary = this->m_modeSampling and 
            this->GetTask()->GetPathConstraints().size() ?
...
SelectTarget() {
  ...
  for(auto formation : this->GetGroupRoadmap()->GetActiveFormations()) {
    auto cfgs = SelectFormationTarget(formation);
    for(auto c : cfgs) {
      auto robot = c.GetRobot();
      used.insert(robot);
      gcfg.SetRobotCfg(robot,std::move(c));
    }
  }
...
FindNearestNeighbor(const GroupCfgType& _cfg, const VertexSet* const _candidates) {
  ...
  auto g = this->GetGroupRoadmap();
  std::cout << "roadmap size: " << g->Size() << std::endl;
...
Extend(const VID _nearVID, const GroupCfgType& _target, GroupLPOutput<MPTraits>& _lp,
  ...
  std::cout << "The extension succeeded. Try to add the node." << std::endl;
