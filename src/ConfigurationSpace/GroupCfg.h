...
class GroupCfg final : public CompositeState<GraphType> {
  ...
  public:
    ...
    bool IsLabel(const std::string& _label, const std::vector<size_t>& _robots={}) const noexcept;
    
    /// Set the robot's string label given the string identifier.
    // void SetLabel(const std::string& _label, const bool _value) noexcept;
    void SetLabel(const std::string& _label, const bool _value, const std::vector<size_t>& _robots={}) noexcept;
    ...
    void GetRandomRay(const double _length, DistanceMetricPointer _dm, Environment* _env,
...
operator+=(const GroupCfg& _other) {
  ...
  if(this->m_formations.size() != _other.m_formations.size()) 
    throw RunTimeException(WHERE) << "Cannot add GroupCfgs with different size of formations. " << this->m_formations.size() << " vs. " << _other.m_formations.size();
  ...
  for(auto formation : this->m_formations) {
    ...
    for(auto robot : formation->GetRobots()) {
      ...
      // std::cout << "cfg: " << this->GetRobotCfg(robot).PrettyPrint() << std::endl;

      // std::cout << "other robot: " << robot->GetLabel() << std::endl;
      // std::cout << "cfg: " << _other.GetRobotCfg(robot).PrettyPrint() << std::endl;
    ...
    // }
    // std::cout << std::endl;
    // std::cout << "otherDofs: " << std::endl;
    // for(auto e : otherDofs) {
    //   std::cout << e << " " ;
    // }
    // std::cout << std::endl;
    ...
    // std::cout << "incremented dofs: " << std::endl;
    // for(auto e : dofs) {
    //   std::cout << e << " " ;
    // }
    // std::cout << std::endl;
    ...
    for(auto cfg : cfgs) {
      // std::cout << "incremented cfg: " << cfg.PrettyPrint() << std::endl;
...
bool
GroupCfg<GraphType>::
IsLabel(const std::string& _label, const std::vector<size_t>& _robots) const noexcept {
  if(_robots.empty()) {
    for(size_t i = 0; i < this->GetNumRobots(); ++i) {
      if(!this->GetRobotCfg(i).IsLabel(_label))
        return false;
    }
  }
  else {
...
void
GroupCfg<GraphType>::
SetLabel(const std::string& _label, const bool _value, const std::vector<size_t>& _robots) noexcept {
  if(_robots.empty()) // Do all robots in this case.
    for(size_t i = 0; i < this->GetNumRobots(); ++i)
      this->GetRobotCfg(i).SetLabel(_label, _value);
  else
    for(size_t i : _robots)
      this->GetRobotCfg(i).SetLabel(_label, _value);
}
...
// void
// GroupCfg<GraphType>::
// GetRandomRay(const double _length, DistanceMetricPointer _dm, Environment* _env, const bool _norm) {
...
//   for(auto formation : m_formations) {
//     auto cfgs = formation->GetRandomFormationCfg(_env->GetBoundary());
//     for(auto cfg : cfgs) {
...
//     // If not, get a random configuration for it.
//     Cfg cfg(robot);
//     cfg.GetRandomRay(_length, _dm, _norm);
...
//   // for(size_t i = 0; i < CompositeDOF(); ++i)
//   //   m_dofs[i] = GRand();
