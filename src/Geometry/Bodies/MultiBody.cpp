...
void
MultiBody::
ConfigureBase(const vector<double>& _v) {
  if(_v == m_currentDofs)
    return;
  std::copy(_v.begin(), _v.begin() + std::min(_v.size(), DOF()),
      m_currentDofs.begin());

  std::cout << "size:: " << m_currentDofs.size() << std::endl;
  // Configure the base.
...
*/
    ...
    if(joint->GetConnectionType() == Connection::JointType::Prismatic) {
      values.push_back(_v[index++]);
    }
    else if(joint->GetConnectionType() == Connection::JointType::Revolute) {
