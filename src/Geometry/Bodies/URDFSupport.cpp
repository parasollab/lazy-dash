...
TranslateURDFJoint(const std::shared_ptr<urdf::Joint>& _joint,
  ...
  if(IsRevolute() || IsSpherical() || IsPrismatic()) {
    double lower_bound = IsPrismatic() ? -1000000 : -1;
    double upper_bound = IsPrismatic() ? 1000000 : 1;

    m_jointRange[0].min = m_jointRange[1].min = lower_bound;
    m_jointRange[0].max = m_jointRange[1].max = upper_bound;
    ...
    for(size_t i = 0; i < numRange; ++i) {
      ...
      double lower = IsPrismatic() ? limits->lower : limits->lower/PI;
      double upper = IsPrismatic() ? limits->upper : limits->upper/PI;
      m_jointRange[i].min = std::max(lower_bound,lower);
      m_jointRange[i].max = std::min(upper_bound,upper);
